...
namespace ADOConsoleApp
{
    class Program
    {
        static string connectionString = "Host=host;Username=user;Password=1234;Database=database";
        ...
        static void FillGiftBoards()
        {
            using (var conn = new NpgsqlConnection(connectionString))
            {
                ...
                for (int i = 1; i <= 10; i++)
                {
                    ...
                    string query = "INSERT INTO GiftBoards (user_id, name, celebration_date, collaborators, access_level, description) VALUES (@user_id, @name, @celebration_date, @collaborators, @access_level, @description)";
                    using (var cmd = new NpgsqlCommand(query, conn))
                    {
        ...
        static void FetchGiftBoards()
        {
            ...
            {
                conn.Open();
                string query = "SELECT * FROM GiftBoards";
